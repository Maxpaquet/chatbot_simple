...
from chatbot.auth.models import TokenResponse, User
from chatbot.services.auth import service_login_for_access_token

router_login = APIRouter()


@router_login.post("/token", response_model=TokenResponse)
async def login_for_access_token(
    form_data: OAuth2PasswordRequestForm = Depends(),
) -> TokenResponse:
    """Authenticate user and return access token."""
    dict_access_token: TokenResponse = await service_login_for_access_token(
